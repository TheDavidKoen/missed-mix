...
export async function action({ request, context, params }: Route.ActionArgs) {
  ...
  if (form.get("intent") === "accept") {
    const outcome = await acceptVibration(env, me, target);
    return outcome === "accepted"
      ? { error: null }
      : { error: "That vibration is no longer waiting." };
  }

  const song = pickSchema.safeParse(parseJson(form.get("song")));
  if (!song.success) return { error: VIBRATION.needSong };

  const outcome = await sendVibration(env, me, target, song.data);
